...
inline void mult_mod_bch4(block* res, block *a, uint64_t *b) {
	...
	for(int i = 0; i < 4; ++i) {
		...
		is[2*i+1] = _mulx_u64(H, b[i], (unsigned long long*)is+8+2*i+1);
		is[2*i] = _mulx_u64(L, b[i], (unsigned long long*)is+8+2*i);
	}
	__m512i t1 = bs[0] & PR8;
...
//	uint64_t h0, h1, l0, l1;
	is[1] = _mulx_u64(H, b, (unsigned long long*)(is+3));
	is[0] = _mulx_u64(L, b, (unsigned long long*)(is+2));
...
inline void mult_mod_bch2(block* res, block *a, uint64_t *b) {
	...
	for(int i = 0; i < 2; ++i) {
		uint64_t H = _mm_extract_epi64(a[i], 1);
		uint64_t L = _mm_extract_epi64(a[i], 0);
		is[2*i+1] = _mulx_u64(H, b[i], (unsigned long long*)is+4+2*i+1);
		is[2*i] = _mulx_u64(L, b[i], (unsigned long long*)is+4+2*i);
	}
	block t1[2], t2[2];
       	t1[0] = bs[0] & prs;
	t1[1] = bs[1] & prs;
...
inline uint64_t mult_mod(uint64_t a, uint64_t b) {
	uint64_t c = 0;
	uint64_t e = _mulx_u64(a, b, (unsigned long long*)&c);
...
inline void mult_mod_bch2(uint64_t *res, uint64_t *a, uint64_t *b) {
	...
	e[0] = _mulx_u64(a[0], b[0], (unsigned long long*)c);
	e[1] = _mulx_u64(a[1], b[1], (unsigned long long*)c+1);
...
inline void mult_mod_bch4(uint64_t *res, uint64_t *a, uint64_t *b) {
	...
	e[0] = _mulx_u64(a[0], b[0], (unsigned long long*)c);
	e[1] = _mulx_u64(a[1], b[1], (unsigned long long*)c+1);
	e[2] = _mulx_u64(a[2], b[2], (unsigned long long*)c+2);
	e[3] = _mulx_u64(a[3], b[3], (unsigned long long*)c+3);
