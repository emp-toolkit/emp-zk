...
//         this->m = m;
//         this->n = n;
//         this->param_matrix = new IntFp[m*(n+1)];
//     }
...
//         // they belong to previous layers
...
//     Layer(LAYER_TYPE type, int input_size, int output_size){
...
//         if(type == AFFINE){
//             this->input = new IntFp[input_size+1];  // +1 for bias
//             this->output = new IntFp[output_size];
//             this->param = new Parameters(output_size, input_size);
//         }

//         if(type != AFFINE){
//             assert(this->input_size == this->output_size && "Input size must match output size for non-affine layers");
//             this->input = new IntFp[input_size]; 
//             this->output = new IntFp[output_size];
//         }
//     }

//     void forward(Layer* prev_layer){
//         // clone the input
...
//     FeedForwardNeuralNetwork(int num_layers, Layer** layers){
...
/****** FLOAT******/
// #ifndef __FEEDFORWARD_FLOAT_H__
// #define __FEEDFORWARD_FLOAT_H__
...
//     ParametersFloat(int m, int n){
//         this->m = m;
//         this->n = n;
//         this->param_matrix = new float[m*(n+1)];
//     }
...
//     ParametersFloat* param;

//     LayerFloat(LAYER_TYPE type, int input_size, int output_size){
//         if(type == INPUT && input_size != output_size){
//             error("Input layer should have same input size and output size!\n");
//         }
...
//         if(type == AFFINE){
//             this->input = new float[input_size+1];  // +1 for bias
//             this->output = new float[output_size];
//             this->param = new ParametersFloat(output_size, input_size);
//         }
...
//     void forward(LayerFloat* prev_layer){
//         // clone the input
//         for(int i = 0; i < input_size; i++){
//             input[i] = float(prev_layer->output[i]);
//         }
//         if(type == AFFINE){
...
// class FeedForwardNeuralNetworkFloat {
//     public: 
...
//     FeedForwardNeuralNetworkFloat(int num_layers, LayerFloat** layers){
//         this->num_layers = num_layers;
//         this->layers = layers;
...
//         // validate layer sizes
//         for(int i = 1; i < num_layers; i++){
//             LayerFloat* prev_layer = layers[i-1];
//             assert(
//                 prev_layer->output_size == layers[i]->input_size && 
//                 "Output size of previous layer must match input size of current layer"
//             );
...
//         int layer_offset = 0;
//         for(int k = 0; k < num_layers; k++){
//             LayerFloat* curr_layer = layers[k];
//             if(curr_layer->type == AFFINE){
//                 curr_layer->param->init_param_matrix(param_file_path, layer_offset);
//                 layer_offset += curr_layer->param->num_parameters();
//             } else {
...
//         LayerFloat* input_layer = layers[0];
...
//     void forward(){
//         LayerFloat* prev_layer = layers[0];
//         for(int i = 1; i < num_layers - 1; i++){
//             // skip INPUT and OUTPUT layers
//             layers[i]->forward(prev_layer);
//             prev_layer = layers[i];
