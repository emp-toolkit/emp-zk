...
void test_single_layer_forward(BoolIO<NetIO> *ios[threads], int party){
    ...
    float* y = new float[sz+1];
    float* z = new float[sz+1];

    // cleartext fixpoint
    int64_t* W_int = new int64_t[sz*(sz+1)];
    int64_t* x_int = new int64_t[sz+1];
    int64_t* y_int = new int64_t[sz+1];

    // EMP fixpoint
    Integer* W_Integer = new Integer[sz*(sz+1)];
    Integer* x_Integer = new Integer[sz+1];
    Integer* y_Integer = new Integer[sz+1];
    Integer* y_res_Integer = new Integer[sz+1];

    // EMP Field
    IntFp* W_IntFp = new IntFp[sz*(sz+1)];
    IntFp* x_IntFp = new IntFp[sz+1];
    IntFp* y_IntFp = new IntFp[sz+1];
    IntFp* z_IntFp = new IntFp[sz+1];
    ...
    // cleartext field
    cout << "LAYER 1 [FLD EMP]:\n";
    affine_layer(sz, sz, W_IntFp, x_IntFp, y_IntFp);
    relu_layer(sz, y_IntFp, y_IntFp);
    normalize(sz, y_IntFp, y_IntFp);
    y_IntFp[sz] = IntFp(1<<FXPSCALE, PUBLIC);
    for(int i = 0; i < sz; i++){
        cout << format_EMP_IntFp(y_IntFp[i], 1) << " ";
    }
    cout << "\n";

    cout << "LAYER 2 [FLD EMP]:\n";
    affine_layer(sz, sz, W_IntFp, y_IntFp, z_IntFp);
    relu_layer(sz, z_IntFp, z_IntFp);
    normalize(sz, z_IntFp, z_IntFp);
    z_IntFp[sz] = IntFp(1<<FXPSCALE, PUBLIC);
    for(int i = 0; i < sz; i++){
        cout << format_EMP_IntFp(z_IntFp[i], 1) << " ";
    }
    cout << "\n";
...
int main(int argc, char** argv){
    ...
    sz = atoi(argv[3]);
    test_single_layer_forward(ios, party);
}