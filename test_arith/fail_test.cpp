...
void test_intfp_mul_check(NetIO *ios[threads + 1], int party)
{
  ...
  batch_reveal_check(c.data(), expect.data(), size);

  cout << "party-" << party << "\t, test_intfp_mul_check, cost time : " << time_from(start) << endl;
  finalize_boolean_zk<NetIO>(party);
}

void test_inner_prdt(NetIO *ios[threads + 1], int party, size_t size) {
  cout << "start to setup....\n";
  auto start = clock_start();
  setup_boolean_zk<NetIO>(ios, threads, party);
  setup_fp_zk<NetIO>(ios, threads, party);
  ...
  fp_zkp_inner_prdt<NetIO>(za.data(), zb.data(), (uint64_t)(PR-2*size), size);
