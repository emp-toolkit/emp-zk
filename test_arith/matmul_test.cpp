...
*/
  // generate seed
//  emp::block seed_block = zero_block;
  emp::PRG prg(&zero_block);
/*  if(party == ALICE) {
    emp::PRG seed_prg;
    prg.random_block(&seed_block);
      ((ZKFpExecPrv<NetIO>*)(ZKFpExec::zk_exec))->io->send_data(&seed_block, sizeof(seed_block));
      sync_boolean_zk();
  } else {
      cout << "verifier recv seed_block...";
      sync_boolean_zk();
      ((ZKFpExecPrv<NetIO>*)(ZKFpExec::zk_exec))->io->recv_data(&seed_block, sizeof(seed_block));
  }
...
#else
 size_t u_size = N/2 + N%2;
  size_t v_size = M/2 + M%2;
  ...
  for (size_t i = 0; i < u_size; i += 2)
  {
    u[i] = IntFp(*(pu+i), PUBLIC);
    if (i+1 >= N)
      break;
    else
    {
      u[i+1] = IntFp(*(pu+i+1), PUBLIC);
    }
  }
#endif
  // public input v
  ...
  {
    v[i] = IntFp(*(pv+i), PUBLIC);
    if (i+1 >= N)
      break;
    else
    {
      v[i+1] = IntFp(*(pv+i+1), PUBLIC);
    }
  }*/
...
void test_intfp_mul_check(NetIO *ios[threads + 1], int party)
{
 /* cout << "start to setup....\n";
...
/*  // without truncation, expect 2* (2^32)
  ...
  */// when finalize zk, it gets "multiplication gates check fails" error
